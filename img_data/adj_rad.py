...
for i in range(100):
    ...
    RC1 = RadialClassifier(r, mit = 1)
    if start == -1: start = RC.get_radius()
    ...
    if FMR > delta: RC.set_opt_rad(-1)
    else: RC.set_opt_rad(1)
...
plt.plot(radius, tmr, 'g-', label="TMR - MAX")
plt.plot(radius, fmr, 'r-', label="FMR - MAX")
