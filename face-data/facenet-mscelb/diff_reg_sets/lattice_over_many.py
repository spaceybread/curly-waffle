...
#        print(r, ma[r])
    ...
    for i in range(3, 11):
        data = get_data('data_files/' + str(i) + '.accum_data_for_lattice.npy')
        
        mem = {}

        for alpha in [2/3, 4/5, 6/7]:
            ma, val = bin_search(data, ALPHA = alpha)
            for v in ma.keys():
            ...
            pd.DataFrame.from_dict(res_ma, orient='columns').to_csv('gaussian_lattice.csv', index=False)
