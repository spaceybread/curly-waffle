...
def first_load(npz_file, text_file, T, R):
...
def test_set(npz_file, T, R):
    ...
    np.save('data_files/' + str(T) + '.accum_data_dist.npy', ma)
    np.save('data_files/' + str(T) + '.accum_data_for_lattice.npy', mb)
...
for i in range(3, 11):
    first_load(sys.argv[1], sys.argv[2], i, R)
    test_set('mapped_data.npy', i, R)
